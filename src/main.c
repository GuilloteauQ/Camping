...
int get_index_free_node(int world_size) {
    ...
    int i = 0;
    while (1) {
        MPI_Request_get_status(reqs[i], &flag, MPI_STATUS_IGNORE);
...
void start_master(char* filename) {
    ...
    while ((read = getline(&line, &len, fp)) != -1) {
        ...
        // printf("%s", line);
        int free_node = get_index_free_node(world_size);
